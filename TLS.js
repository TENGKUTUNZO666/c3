/* note. jangan mengubah codingan karena bisa mengakibatkan error */
...
console.log('tidak ada proxy. tambahkan proxy dulu. bodoh');
process.exit();
}
...
var parsed = url.parse(objetive);
    ...
    console.log('Method TLS by Levyxnet!!!');
    console.log('node TLS.js [URL] [TIME]');
    console.log('node TLS.js https://examples.com 200');
    process.exit();
}
const sigalgs = [
...
var UAs = fs.readFileSync('UA.txt', 'utf-8').replace(/\r/g, '').split('\n');
     ...
     console.log('tidak ada useragents. silahkan tambahkan usergents dulu. bodoh')
 }
class TlsBuilder {
    constructor (socket){
   ...
   ╚══════╝╚══════╝  ╚═══╝     ╚═╝   ╚═╝  ╚═╝`);
    ...
    http2TUNNEL(socket){
        ...
        payload["Cache-Control"] = 'no-cache, no-store,private, max-age=500000, must-revalidate';
        payload["Pragma"] = 'no-cache, no-store,private, max-age=500000, must-revalidate';
        payload['client-control'] = 'max-age=500000, s-max-age=500000';
